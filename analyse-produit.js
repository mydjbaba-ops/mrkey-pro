...
export default async function handler(req, res) {
  ...
  try {
    ...
    const titre = titleMatch
      ? titleMatch[1].trim()
          .split(" - ")[0].split(" | ")[0]
          .replace(/&amp;/gi, "&").replace(/&nbsp;/gi, " ").replace(/&#\d+;/gi, "").replace(/&[a-z]+;/gi, "")
          .replace(/\s+/g, " ").trim()
      : "";
    ...
    const wooImageMatch = html.match(/class=["'][^"']*wp-post-image[^"']*["'][^>]+src=["']([^"']+)["']/i) ||
                          html.match(/<img[^>]+src=["'](https?:\/\/[^"']+\/storage\/images\/products\/gallery\/[^"']+\.(?:jpg|jpeg|png|webp))["']/i) ||
    ...
    // Marque : chercher après "pour" en priorité, sinon première occurrence dans le titre
    const titreLow = titre.toLowerCase();
    const pourMarqueMatch = titreLow.match(/pour\s+([a-zà-ÿ]+)/i);
    let marque = "";
    if (pourMarqueMatch) {
      marque = MARQUES.find(m => pourMarqueMatch[1].toLowerCase() === m.toLowerCase()) || "";
    }
    if (!marque) marque = MARQUES.find(m => titreLow.includes(m.toLowerCase())) || "";
    ...
    if (compatHtml) {
      ...
      modeles = [...modelesSet].map(m => m.replace(/&[a-z]+;/gi, " ").replace(/\s+/g, " ").trim()).join(", ");
    }
    ...
    const freq = freqMatch
      ? freqMatch[0].replace(",", ".").replace(/\s/g, "").replace(/mhz/i, "MHz")
      : "";

    // PCF = uniquement PCF\d+ (jamais ID46 ici)
    const pcfMatch = texte.match(/\b(pcf\s?\d{4,})\b/i);
    const transpondeur = pcfMatch ? pcfMatch[0].toUpperCase().replace(/\s/g, "") : "";
    const pcf = transpondeur;

    const lameMatch = texte.match(/\b(va\d+|hu\d+|sip\d+|nf\d+|ne\d+|net\d+|hu83|hu66|hu64|hu100|hu101|hu92|hu46|sip22|ft22|toy\d+|nef\d+|hfu\d+|ya\d+|rn\d+|df\d+|vac\d+|hy\d+|fo\d+)\b/i);
    const lame = lameMatch ? lameMatch[0].toUpperCase() : "";

    const pileMatch = texte.match(/\b(cr2032|cr2025|cr2016|cr1620|cr1632|cr1616|cr1225|lr44|a76)\b/i);
    const pile = pileMatch ? pileMatch[0].toUpperCase() : "";

    // Boutons — chercher "X boutons" ou "boutons: X" dans le texte
    const boutonsMatch = texte.match(/(?:boutons?|buttons?)\s*:?\s*(\d)/i) ||
                         texte.match(/(\d)\s*(?:bouton|button|touche)/i);
    ...
    else if (texte.includes("smart key") || texte.includes("keyless go") || texte.includes("mains libres")) type = "Smart Key";
    else if (texte.includes("card key") || texte.includes("clé carte") || texte.includes("smart carte")) type = "Carte";
    else if (texte.includes("coque") || texte.includes("boitier vide") || texte.includes("shell")) type = "Coque";
    else if (texte.includes("transpondeur chip") || texte.includes("puce seule") || texte.includes("transponder only")) type = "Transpondeur";

    const prixMatch = html.match(/(\d+[.,]\d{2})\s*€/);
    const prix = prixMatch ? parseFloat(prixMatch[1].replace(",", ".")) : 0;

    // Numéros de pièce — extraire depuis la page
    let ref = "";
    // Cherche dans le texte brut après "Numéros de pièce" ou "Numéro de pièce"
    const refTexteMatch = texte.match(/num[eé]ros?\s+de\s+pi[eè]ce\s*:?\s*([a-z0-9][a-z0-9\s\/,.-]{4,120}?)(?:\s+(?:fr[eé]quence|transpondeur|lame|pile|bouton|marque|garantie|état|clé|batterie))/i);
    if (refTexteMatch) {
      ref = refTexteMatch[1].replace(/&[a-z]+;/gi, " ").replace(/\s+/g, " ").trim().slice(0, 100);
    }
    if (!ref) {
      // Cherche dans le HTML structuré
      const refHtmlMatch = html.match(/(?:num[eé]ros?\s+de\s+pi[eè]ce|part\s+number)[^<]{0,30}:?\s*<[^>]*>([^<]{5,120})/i);
      if (refHtmlMatch) ref = refHtmlMatch[1].replace(/<[^>]+>/g, "").replace(/&[a-z]+;/gi, " ").replace(/\s+/g, " ").trim().slice(0, 100);
    }
    if (!ref) {
      // Fallback : slug URL (4 premiers segments)
      const urlParts = url.split("/").filter(Boolean);
      const refSlug = urlParts[urlParts.length - 1] || "";
      ref = refSlug.split("-").slice(0, 4).join("-").toUpperCase().slice(0, 20);
    }
